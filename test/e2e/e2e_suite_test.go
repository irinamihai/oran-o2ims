...
var _ = Describe("Dry-run-ProvisioningRequestReconcile", func() {
    ...
    defaultHugepagesSize: "1G"`,
        ...
        Eventually(func(g Gomega) {
            ...
            K8SClient.Get(context.Background(), client.ObjectKeyFromObject(ct), newct)
            g.Expect(newct.Status.Conditions).ToNot(BeNil())
	...
	Context("Resources preparation during initial Provisioning", func() {
		It("Verify status conditions if ClusterInstance rendering fails", func() {
			...
			Eventually(func(g Gomega) {
				err := K8SClient.Get(ctx, client.ObjectKeyFromObject(ProvRequestCR), reconciledPR)
                Expect(err).ToNot(HaveOccurred())
				g.Expect(reconciledPR.Status.Conditions).ToNot(BeNil())
			}, timeout, interval).Should(Succeed())
