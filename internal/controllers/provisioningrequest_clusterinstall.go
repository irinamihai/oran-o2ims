...
func (t *provisioningRequestReconcilerTask) renderClusterInstanceTemplate(
	...
	renderedClusterInstanceUnstructure := &unstructured.Unstructured{}
    renderedClusterInstanceUnstructure.SetAPIVersion(fmt.Sprintf("%s/%s", siteconfig.Group, siteconfig.Version))
    renderedClusterInstanceUnstructure.SetKind(siteconfig.ClusterInstanceKind)
    renderedClusterInstanceUnstructure.SetName(t.clusterInput.clusterInstanceData["clusterName"].(string))
    renderedClusterInstanceUnstructure.SetNamespace(t.clusterInput.clusterInstanceData["clusterName"].(string))

	err := unstructured.SetNestedField(renderedClusterInstanceUnstructure.Object, t.clusterInput.clusterInstanceData, "spec")
	if err != nil {
		return nil, fmt.Errorf(
			"failed to set the unstructured spec for the %s(%s) CRD: %w",
			renderedClusterInstanceUnstructure.GetName(), renderedClusterInstanceUnstructure.GetNamespace(), err)
	}
    ...
    nodes := renderedClusterInstanceUnstructure.Object["spec"].(map[string]interface{})["nodes"].([]interface{})
    for _, node := range nodes {
        // Set placeholders for BMC details.
        nodeMapIntf := node.(map[string]interface{})
		nodeMapIntf["bmcAddress"] = "placeholder"
		nodeMapIntf["bootMACAddress"] = "00:00:5E:00:53:AF"
    }
