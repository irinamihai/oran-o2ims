...
func removeLabelFromInterfaces[T any](data T) error {
	...
	if ok {
		for _, node := range nodes {
			...
			for _, intf := range interfaces {
				if value, exists := intf["label"]; exists && value != "" {
					delete(intf, "label")
				}
			}
		}
...
func removeRequestedFromClusterInstanceSchema(schema map[string]any) {
		...
		if propsMap, ok := properties.(map[string]any); ok {
			for _, propValue := range propsMap {
				if propSchema, ok := propValue.(map[string]any); ok {
					removeRequestedFromClusterInstanceSchema(propSchema)
				}
			}
		}
	}
	...
	if items, hasItems := schema["items"]; hasItems {
		if itemSchema, ok := items.(map[string]any); ok {
			removeRequestedFromClusterInstanceSchema(itemSchema)
		}
	}
...
func ValidateDefaultConfigmapSchema[T any](ctx context.Context, c client.Client, data T) error {
	// Get the ClusterInstance CRD as a 
	...
	for index, version := range versions {
		...
		if err != nil || !found {
			return fmt.Errorf("failed to obtain the openAPIV3Schema from version %d of the %s.%s CRD: %w", index, ClusterInstanceCrdName, siteconfig.Group, err)
		}
		break
	}
	...
	// Remove the `requested` property as the default ConfigMaps contains only a subset of the ClusterInstance spec.
	removeRequestedFromClusterInstanceSchema(openAPIV3SchemaSpec)
	// Dissalllow unknown properties in the ClusterInstance CRD schema.
	...
	dataMap, _ := any(data).(map[string]any)
	removeLabelFromInterfaces(dataMap)

	err = provisioningv1alpha1.ValidateJsonAgainstJsonSchema(openAPIV3SchemaSpec, dataMap)
	if err != nil {
		return fmt.Errorf(
			"ClusterInstance default ConfigMap does not match the ClusterInstance schema: %w", err)
	}
	return nil
}
