...
//   - editable : the values for configuration is it exposed through the ClusterTemplate and
//     can later be changed through the ProvisioningRequest
//
// If any error is encountered, the default data is return as it is in the ConfigMap, without
...
func GetDefaultsFromConfigMap(ctx context.Context, c client.Client, configMapName string, configMapNamespace string,
	...
	// Variables to keep the ConfigMaps.
	...
	// Get the schema to check the default values against.
...
// elements that do not pass the schema.
